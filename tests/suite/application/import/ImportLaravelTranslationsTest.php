...
class ImportLaravelTranslationsTest extends TestCase
{
    ...
    {
        config()->set('squanto.lang_path', $this->getFixtureDirectory('lang'));

        app()->bind(LaravelTranslationsReader::class, function ($app) {
            return new LaravelTranslationsReader(
                new Filesystem(new Local($this->getFixtureDirectory('lang')))
            );
        });
    }
    ...
    {
        config()->set('squanto.lang_path', $this->getFixtureDirectory('langchanged'));

        app()->bind(LaravelTranslationsReader::class, function ($app) {
            return new LaravelTranslationsReader(
                new Filesystem(new Local($this->getFixtureDirectory('langchanged')))
            );
        });
    }
}