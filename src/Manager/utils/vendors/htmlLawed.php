...
function htmLawed($t, $C=1, $S=array())
{
    ...
    $C['deny_attribute'] = $x;
    // config URL
    $x = (isset($C['schemes'][2]) && strpos($C['schemes'], ':')) ? strtolower($C['schemes']) : 'href: aim, feed, file, ftp, gopher, http, https, irc, mailto, news, nntp, sftp, ssh, telnet, tel; *:file, http, https';
    $C['schemes'] = array();
...
function hl_bal($t, $do=1, $in='div')
{
    ...
    for($i=-1, $ci=count($t); ++$i<$ci;){
        ...
        // close tag
        if($s) {
            if(isset($cE[$e]) or !in_array($e, $q)) {continue;
            } // Empty/unopen
            if($p == $e) {array_pop($q); echo '</', $e, '>'; unset($e); continue;
            } // Last open
...
function hl_prot($p, $c=null)
{
    ...
    if(isset($c['*']) or !strcspn($p, '#?;') or (substr($p, 0, 7) == $d)) {return "{$b}{$p}{$a}";
...
function hl_tag($t)
{
    ...
    // attr name-vals
    if(strpos($a, "\x01") !== false) {$a = preg_replace('`\x01[^\x01]*\x01`', '', $a);
